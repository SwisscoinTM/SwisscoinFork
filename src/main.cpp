...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > 303460 && nHeight < 356350)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nHeight));

    if (IsProofOfStake() && nHeight <= Params().LastPOWBlock() && nHeight > 303255 && nHeight < 303460)
        return DoS(100, error("AcceptBlock() : reject proof-of-stake at height %d", nHeight));

    if (IsProofOfStake() && nHeight <= Params().LastPOWBlock() && nHeight > 356350)
        return DoS(100, error("AcceptBlock() : reject proof-of-stake at height %d", nHeight));
